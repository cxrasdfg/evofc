...
from utils.utils import create_disjoint_indices, split_dataset as util_split_dataset

IMAGENET_DIR = os.path.expanduser('~/work/datasets/imagenet')
...
def scan_ilsvrc_dir(args, num_process = 4):
    ...
    trn_dir = os.path.join(IMAGENET_DIR, 'train')
...
def create_evolved_dataset_dali(args):
    ...
    tst_dir = os.path.join(IMAGENET_DIR, 'val')
...
def create_prune_dataset_tv(args):
    pip_train = datasets.ImageFolder(
            os.path.expanduser(f'{IMAGENET_DIR}/train/'),
            TRAIN_TRANS_TV)
    
    pip_test = datasets.ImageFolder(os.path.expanduser(f'{IMAGENET_DIR}/val/'), TEST_TRANS_TV)
    return pip_train, pip_test
...
def create_evolved_dataset_tv(args):
    ...
    test_set = datasets.ImageFolder(os.path.expanduser(f'{IMAGENET_DIR}/val/'), TEST_TRANS_TV)

    train_set, val_set = util_split_dataset(train_dataset_1, train_dataset_2, args.train_val_ratio)
