...
class InvertedResidual(nn.Module):
    def __init__(
        self, inp: int, cfg:int,  oup: int, stride: int, expand_ratio: int, norm_layer: Optional[Callable[..., nn.Module]] = None
    ) -> None:
        super().__init__()
        self.stride = stride
...
class MobileNetV2(nn.Module):
    def __init__(
        ...
        if cfg is None:
            # default cfg shoul be:
            # [32, 96, 144, 144, 192, 192, 192, 384, 384, 384, 384, 576, 576, 576, 960, 960, 960]    
            cfg = [] # each elem represents the hidden dim.
            for t, c, n, s in inverted_residual_setting:
                output_channel = _make_divisible(c * width_mult, round_nearest)
                for i in range(n):
                    stride = s if i == 0 else 1
                    # features.append(block(input_channel, output_channel, stride, expand_ratio=t, norm_layer=norm_layer))
                    cfg.append(int(round(input_channel * t)))
                    input_channel = output_channel   

        
        # import pdb; pdb.set_trace()
        # cfg = [32, 9, 14, 15, 19, 20, 21, 22, 38, 39, 40, 57, 58, 59, 96, 97, 98]
        self.cfg = cfg
        ...
        for t, c, n, s in inverted_residual_setting:
            ...
            for i in range(n):
                stride = s if i == 0 else 1
                features.append(block(input_channel, cfg[cfg_idx], output_channel, stride, expand_ratio=t, norm_layer=norm_layer))
                input_channel = output_channel
                cfg_idx += 1
