...
def snip_forward_conv2d(self, x):
        return F.conv2d(x, self.weight * self.weight_mask_snip, self.bias,
                        self.stride, self.padding, self.dilation, self.groups)


def snip_forward_linear(self, x):
        return F.linear(x, self.weight * self.weight_mask_snip, self.bias)
...
def get_all_criterion(net, train_dataloader, device, prune_probs):
    ...
    # net = copy.deepcopy(net) # we do not care about this error.

    # Monkey-patch the Linear and Conv2d layer to learn the multiplicative mask
    # instead of the weights
    # net=copy.deepcopy(net.module)
    net=net.module
    
    original_conv_forward_list = []
    # register the forward function to compute the indicator.
    ...
    if hasattr(net, 'module'):
        net_modules = net.module.modules()
    else:
        net_modules = net.modules()
    for layer in net_modules:
        if isinstance(layer, nn.Conv2d):
            grads_abs.append(torch.abs(layer.weight_mask_es.grad))
