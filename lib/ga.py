...
class EvoPrune(GeneticAlgorithm):
    def __init__(self, **kwargs):
        kwargs['individial'] = Individual(rank=np.inf, crowding=-1)
        super(EvoPrune, self).__init__(**kwargs)
...
def evoprune(pop_size,
            sampling=RandomSampling(var_type=np.float32),
            selection=TournamentSelection(func_comp=binary_tournament),
            crossover=PointCrossover(n_points=2),
            mutation=PolynomialMutation(eta=3, var_type=np.float32),
    ...
    """
    return EvoPrune(pop_size=pop_size, sampling=sampling,
                    selection=selection, crossover=crossover,
                    mutation=mutation, survival=RankAndCrowdingSurvival(),
                    eliminate_duplicates=eliminate_duplicates,
                    n_offsprings=n_offsprings,**kwargs)

parse_doc_string(evoprune)